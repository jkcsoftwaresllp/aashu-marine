...
/**
 * Unit Tests for Service_Card Component
 * Feature: landing-page-home
 * **Validates: Requirements 9.1, 9.4**
 * 
 * These tests verify specific examples and edge cases for the Service_Card component.
 */

describe('Service_Card Unit Tests', () => {
  /**
   * Test rendering with specific dummy data from services array
   ...
   */
  it('renders correctly with specific dummy data from services array', () => {
    const serviceData = services[0]; // First service from dummy data
    ...
    expect(heading).toHaveTextContent(serviceData.heading);
    
    // Verify description is rendered with correct text
    expect(screen.getByText(serviceData.description)).toBeInTheDocument();
    
    // Verify icon container exists
    const iconContainer = container.querySelector('.service-card__icon');
    expect(iconContainer).toBeInTheDocument();
  });

  /**
   * Test rendering with all services from dummy data
   * Ensures the component works with all service entries in the data array.
   */
  it('renders correctly with all services from dummy data', () => {
    services.forEach((service) => {
      ...
      expect(screen.getByRole('heading', { level: 3 })).toHaveTextContent(service.heading);
      expect(screen.getByText(service.description)).toBeInTheDocument();
      expect(container.querySelector('.service-card__icon')).toBeInTheDocument();
  ...
  it('handles empty string props gracefully', () => {
    ...
    expect(container.querySelector('.service-card__heading')).toBeInTheDocument();
    expect(container.querySelector('.service-card__description')).toBeInTheDocument();
    expect(container.querySelector('.service-card__icon')).toBeInTheDocument();
  });
  ...
  it('renders icon as img element when icon is a string', () => {
    const { container } = render(
      <Service_Card
        ...
        heading="Test Service"
        description="Test Description"
      />
    );
    
    const img = container.querySelector('.service-card__icon img');
